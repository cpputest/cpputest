...
SimpleString StringFrom(MemoryBufferContainer const &value)
{
    ...
    size_t i;
    uint8_t const *dataBuffer = (uint8_t const *)value.getBuffer();
    size_t dataBufferLen = value.getBufferLength();
    size_t dataBufferToPrint = dataBufferLen;

    if (dataBufferLen > maxDataBufferToPrint) {
        dataBufferToPrint = maxDataBufferToPrint;
    }
    for (i = 0; i < dataBufferLen; i++) {
        dataString += StringFromFormat("%02X ",dataBuffer[i]); 
    }
    if (dataBufferToPrint < dataBufferLen) {
        dataString += SimpleString("..."); 
    }

    return SimpleString("Length: ") + StringFrom((unsigned int)dataBufferLen) + SimpleString(", Data: ") + dataString;
}
